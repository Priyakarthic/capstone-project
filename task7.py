...
connection = mysql.connector.connect(
  ...
  port = 4000,
  user = "t5YFaLGJwn44ZiZ.root",
  password = "aoJRERvQ8qp9sum4",
...
def missing_data(census_2011):
    st.title('Missing Data Visualization')
    df = pd.read_excel('census_2011.xlsx', index_col=0)
    st.dataframe(df)

# Window 1 : Total population of each District

def window_1():
    window('Total population of each District',"SELECT District_Code, State_name, District_name, Population FROM census.census_data")

# Window 2 : Literate male and female in each district

def window_2():
    window('Literate Male and Female in each district', \
            "SELECT District_Code, State_name, District_name, Male_Literate, Female_literate FROM census.census_data")

# window 3 : Percentage of workers in each district

def window_3():
    window('Percentage of Workers in each district',
            "SELECT District_Code, State_name, District_name, ROUND((Workers/Population)*100, 2) \
                            AS Percentage_of_Workers FROM census.census_data")
...
def window_4():
    window('Households with LPG or PNG as Cooking fuel',
            "SELECT District_Code, State_name, District_name, LPG_or_PNG_Households FROM census.census_data")
...
def window_5():
    window("Religious composition of each district (in percentage)",
            "SELECT District_Code, State_name, District_name, \
...
def window_6():
    window("Households with Internet in each District_name", 
            "SELECT District_Code, State_name, District_name, Households_with_Internet FROM census.census_data")
...
def window_7():
    window('Educational attainment distribution in each district',
            'SELECT District_Code, State_name, District_name, Below_Primary_Education, Primary_Education, Middle_Education, \
                Secondary_Education, Higher_Education, Graduate_Education, Other_Education, Literate_Education, \
                Illiterate_Education, Total_Education FROM census.census_data')
...
def window_8():
    window('Households with access to various modes of transports in each district', 
            'SELECT District_Code, State_name, District_name, Households_with_Bicycle, Households_with_Car_Jeep_Van, \
                Households_with_Radio_Transistor, Households_with_Scooter_Motorcycle_Moped FROM census.census_data')
...
def window_9():
    window('Condition of occupied census houses', 
            ...
            Not_having_lat_fac_within_preid_alter_source_open_houhol  AS Houses_without_latrine_facility \
            FROM census.census_data')
...
def window_10():
    window('Distribution of Household size',
            'SELECT District_Code, State_name, District_name, Household_size_1_person_Households AS 1_Person, \
...
def window_11():
    window('Total number of households in each state',
            'SELECT State_name, SUM(Households) AS Total_Households \
            FROM census.census_data \
            GROUP BY State_name')
...
def window_13():
    window('Average household size in each state',
            'SELECT State_name, ROUND(AVG(Household_size_1_person_Households), 2) AS Average_of_1_person_houses, \
...
def window_14():
    window('Owned versus rented houses in each state',
            'SELECT State_name, SUM(Ownership_Owned_Households) AS Owned_Houses, SUM(Ownership_Rented_Households) AS Rented_Houses \
            FROM census.census_data \
            GROUP BY State_name')
...
def window_15():
    window('Distribution of different types of latrine facilities in each state',
            'SELECT State_name, SUM(Type_of_latrine_facility_Pit_latrine_Households) AS Pit_Latrine, \
            SUM(Type_of_lat_fac_Nit_soil_dispose_into_open_drain_houhol ) AS Night_soil_disposed_into_open_drain_Latrine, \
            SUM( Type_of_latrine_fac_pour_flush_latrine_conn_other_sys_houhol) AS Flush_or_Pour_Latrine, \
...
def window_17():
    window('Average household income distribution in each state based on the power parity categories',
            'SELECT State_name, ROUND(AVG(Power_Parity_Less_than_Rs_45000), 2) AS Avg_Power_Parity_Less_than_Rs_45000, \
...
def window_19():
    window('Households below the poverty line in each state',
            'SELECT State_name, SUM(Power_Parity_Less_than_Rs_45000) AS Households_below_poverty_line FROM census.census_data \
            GROUP BY State_name')
...
def main():
    st.title("Census 2011 Database Query")
    ...
    select = st.selectbox("Select a topic to display :",options = options, index = None, placeholder = 'Choose an option')
    if select == 'Missing Data Visualization':
        missing_data('census_2011.xlsx')
